...
ONNX -> TFLite conversion (only when --model is .onnx):
  ...
  --keep-gridsample-border   Disable the default GridSample(border) ->
                             Clamp + GridSample(zeros) rewrite. onnx2tf only
                             supports padding_mode='zeros'; only set this if
                             you've patched onnx2tf yourself.
...
while [ $# -gt 0 ]; do
    case "$1" in
        ...
        --keep-gridsample-border) OVR_KEEP_GS_BORDER=1; shift 1;;
    ...
    tflite)
        if [ "$ext" = "tflite" ]; then
            ...
            if [ ! -f "$TFLITE" ]; then
                ...
                [ "$OVR_KEEP_GS_BORDER" = "1" ]  && CONVERT_FLAGS+=(--keep-gridsample-border)
