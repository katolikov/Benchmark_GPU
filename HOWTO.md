...
| `TypeError: ... ConcatV2 ... types [int32, int64] don't all match` <br> `TypeError: ... must have the same dtype, got float16 != float32` | ONNX permits mixed dtypes on element-wise ops; TF doesn't. The converter (a) folds constants via `onnxsim`, then (b) runs topological dtype propagation (Cast / Constant / Shape outputs are tracked explicitly) and inserts `Cast` nodes before any mixed-dtype op (int64→int32 for shape ops, fp16→fp32 elsewhere). If you still see the error — most often when onnx2tf re-introduces an fp16 boundary internally — re-run with **`--force-fp32`**: that strips every fp16/bf16 from the ONNX graph (lossless upcast) before conversion, so no fp16 boundary can possibly remain. Both flags are exposed in `scripts/run_bench.sh` as `--force-fp32` / `--convert-verbose`. |
